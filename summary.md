# Session 7 Checkpoint - Key Takeaways
...
optimistic on imbalanced data. PR-AUC reflects how well
...
`temperature_2m` and on `wind_speed_10m` / `wind_speed_100m` tell us that collinearity is present here between these pairs (obviously). We can still conclude that **heat
...
The **ranking flips between metrics**: logistic wins on PR-AUC, RF
...
**Per-fold winners (PR-AUC, best variant of each class) — the models
...
A 2-2-1 split across model classes. Logistic wins
...
all-history by **+0.05 PR-AUC**. The 3-year
...
the test distribution much more closely. This suggest that there have been some changes to the grid. Post-Uri ERCOT volatility has subsided, more transmission and renewables are online, and hourly spike dynamics in 2024-2026 do not look like 2021-2022. Worth noting
that ROC-AUC barely moves across windows (0.90 → 0.90 → 0.90) — the ranking
ability is preserved, what changes is the precision-vs-recall tradeoff.

## 5. Open questions for the final checkpoint

1. **Spike magnitude prediction (regression task).** For the final, we shift to predicting magnitude conditional on a spike occurring. So I have to fit a regression model on the spike-only subset, using the same features.
...
2. **Strongest predictors and articulation.** Logistic and RF agree on the
   *headline four* — temperature, apparent temperature, gas price, hour —
   but disagree on the rest. So I need to figure out how to reconcile this. 
