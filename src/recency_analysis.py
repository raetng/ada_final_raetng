"""Does training on more recent data help, hurt, or make no difference?

Holds out the most recent 12 months as a fixed test set, then refits the best
Random Forest configuration (from train_random_forest.py) on three different
training windows ending at the test-set boundary:

  - all available history
  - the most recent 3 years before the test set
  - the most recent 1 year before the test set

If recency helps, the project narrative becomes "the grid has changed and
recent data captures that". If it doesn't, the narrative is "the relationships
are stable enough that more data wins."

The bonus probability-distribution plot is a calibration sanity check: if older
training windows produce probabilities that are systematically shifted vs a
recent-only model, that's visible distributional drift.
"""
...
# Best RF variant from train_random_forest.py: depth=None_leaf=20.
RF_PARAMS = dict(
    n_estimators=300,
    n_jobs=-1,
    random_state=42,
...
def plot_pr_auc_bars(window_results, output_path):
    ...
    ax.set_title("Random Forest: training-window recency vs PR-AUC")
...
def plot_probability_distributions(window_probas, y_test, output_path):
    ...
    fig.suptitle("Test-set probability distributions by training window", fontsize=12)
    fig.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
