"""STAGE 2 Random Forest regressor on log(SPP) | SPP > $100 (HB_HOUSTON).

Non-parametric, handles non-linearity and feature interactions natively. After
the OLS baseline produced negative R^2 and Lasso degenerated to a near-mean
predictor, this is the first model in the stage-2 ladder that can plausibly
capture interactions (e.g., gas-price effect being conditional on temperature).

Grid (small, since CV is chronological and we don't want to claim tuning
results swamped by fold-to-fold variance):
  max_depth in {10, None}  x  min_samples_leaf in {5, 20}  = 4 variants
...
RESULTS_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "rf_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "rf_best_predictions.npz"
IMP_PLOT_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "rf_feature_importance.png"
MODEL_PATH = PROJECT_ROOT / "models" / "stage2_rf_best_houston.pkl"
...
def main():
    print("=" * 70)
    print("STAGE 2: RANDOM FOREST REGRESSOR (HB_HOUSTON)")
    print("=" * 70)
    ...
    imp_df = plot_feature_importance(
        ...
        f"Stage 2 RF {best_name} (HB_HOUSTON) — feature importance"
    )
