"""STAGE 2 baseline: OLS linear regression of log(SPP) | SPP > $100 (HB_HOUSTON).

Second leg of the two-stage spike-prediction setup. Stage 1's job (in
src/train_*.py and src/train_*_multihub.py) is the binary classification
"is this hour a spike?". Stage 2's job is "given that it is a spike, how big?".

Why log(SPP): spike magnitudes have a heavy right tail (median $160, max
~$9000 in HB_Houston). OLS on raw dollars would be dominated by a handful of
extreme hours. Working in log-space turns multiplicative errors into additive
ones and keeps the loss balanced across the price range.

Why an unregularized OLS baseline: this is the simplest sensible model in the
stage-2 ladder. More complex regressors (RF / KNN / regularized linear) will
be compared back to this baseline once we add them.

5-fold expanding-window time-series CV mirrors the stage-1 framework so the
two stages can be combined without worrying about train/test alignment.
"""
...
RESULTS_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "linear_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "linear_predictions.npz"
COEF_PLOT_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "linear_coefficients.png"
SCATTER_PLOT_PATH = PROJECT_ROOT / "outputs" / "stage2" / "houston" / "predicted_vs_actual.png"
MODEL_PATH = PROJECT_ROOT / "models" / "stage2_linear_baseline_houston.pkl"
...
def run_cv(X, y, df, splits):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        print(
            f"  fold {fold_idx}: train={len(X_train):4d}  test={len(X_test):4d}  "
...
def plot_coefficients(model, feature_names, output_path):
    ...
    ax.set_title("Stage 2 OLS (HB_HOUSTON) — coefficients on standardized features")
...
def plot_predicted_vs_actual(y_true_log, y_pred_log, output_path):
    ...
    ax.set_title("Stage 2 OLS (HB_HOUSTON) — out-of-fold predictions")
    ax.legend()
...
def main():
    print("=" * 70)
    print("STAGE 2: LOAD MAGNITUDE-REGRESSION DATA (HB_HOUSTON)")
    print("=" * 70)
