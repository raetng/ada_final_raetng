"""STAGE 2 regularized-linear: Ridge and Lasso on log(SPP) | SPP > $100 (multi-hub).

Multi-hub analog of stage2_train_regularized_linear.py. Same alpha grid;
uses the timestamp-grouped time-series splitter so fold boundaries respect
multi-hub timestamps.
"""
...
RESULTS_PATH = PROJECT_ROOT / "outputs" / "stage2" / "multihub" / "regularized_linear_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "stage2" / "multihub" / "regularized_linear_best_predictions.npz"
COEF_PLOT_PATH = PROJECT_ROOT / "outputs" / "stage2" / "multihub" / "regularized_linear_coefficients.png"
MODEL_PATH = PROJECT_ROOT / "models" / "stage2_regularized_linear_best_multihub.pkl"
...
def main():
    print("=" * 70)
    print("STAGE 2: REGULARIZED LINEAR (MULTI-HUB)")
    print("=" * 70)
    ...
    coef_df = plot_coefficients(
        ...
        f"Stage 2 {best_name} (multi-hub) — coefficients"
    )
