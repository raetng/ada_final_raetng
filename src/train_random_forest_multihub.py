"""Train Random Forest models for spike prediction (multi-hub).

Multi-hub analog of train_random_forest.py. Same 3x3 grid (max_depth x
min_samples_leaf) and same selection-by-mean-per-fold-PR-AUC; the only
differences are the dataset (pooled HB_* hubs) and the grouped time-series
splitter, both of which keep this comparable to the Houston-only run while
avoiding hub-to-hub leakage at fold boundaries.
"""
...
from evaluation import evaluate_classifier, make_time_series_splits_grouped

PROJECT_ROOT = Path(__file__).resolve().parent.parent
DATA_PATH = PROJECT_ROOT / "data" / "processed" / "modeling_data_multihub.parquet"
RESULTS_PATH = PROJECT_ROOT / "outputs" / "final" / "rf_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "final" / "rf_best_predictions.npz"
IMP_PLOT_PATH = PROJECT_ROOT / "outputs" / "final" / "rf_feature_importance.png"
MODEL_PATH = PROJECT_ROOT / "models" / "rf_best_multihub.pkl"
...
def plot_feature_importance(model, feature_names, output_path):
    ...
    ax.set_title("Random Forest feature importance (multi-hub, best variant)")
...
def main():
    print("=" * 70)
    print("LOAD MODELING DATA (MULTI-HUB)")
    print("=" * 70)
