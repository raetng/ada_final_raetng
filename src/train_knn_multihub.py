"""Train KNN classifiers for spike prediction (multi-hub).

Multi-hub analog of train_knn.py. KNN cost scales roughly linearly with the
training-set size for each prediction, so pooling four hubs is expected to
take ~4x longer per fold than the Houston-only run. The k sweep, weighting,
and selection criterion are otherwise identical.
"""
...
RESULTS_PATH = PROJECT_ROOT / "outputs" / "final" / "knn_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "final" / "knn_best_predictions.npz"
MODEL_PATH = PROJECT_ROOT / "models" / "knn_best_multihub.pkl"
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"{N_SPLITS}-FOLD EXPANDING-WINDOW CV (TIMESTAMP-GROUPED)  (k = {K_VALUES})")
    print("=" * 70)
