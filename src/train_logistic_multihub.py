"""Train logistic-regression baselines for spike prediction (multi-hub).

Multi-hub analog of train_logistic.py. Uses the pooled dataset over the four
real HB_* hubs and the grouped time-series splitter so fold boundaries land
between timestamps (avoiding hub-to-hub leakage at the cut).

Grid, scaling, selection criterion, and outputs mirror the Houston-only
...
RESULTS_PATH = PROJECT_ROOT / "outputs" / "final" / "logistic_results.json"
PRED_PATH = PROJECT_ROOT / "outputs" / "final" / "logistic_best_predictions.npz"
MODEL_PATH = PROJECT_ROOT / "models" / "logistic_best_multihub.pkl"
