"""Compare logistic, random forest, and KNN on the common evaluation framework.

Loads each model class's per-variant results JSON, picks the winner inside
each class by PR-AUC, and produces:

  - outputs/session7/model_comparison.md  — markdown table sorted by PR-AUC
  - outputs/session7/pr_curves_comparison.png
  - outputs/session7/roc_curves_comparison.png

Then walks fold-by-fold to report which model class wins each chronological
test window, and prints a short interpretation that weighs the headline gap
against the cross-fold standard deviation.
"""
...
def load_best_per_class():
    """For each model class, load all variants and return the one with highest mean
    per-fold PR-AUC. All three training scripts (logistic, RF, KNN) now use this same
    mean-per-fold criterion to pick their saved-model winner, so the comparison's
    "best variant" agrees with the model in models/*.pkl by construction."""
    best = {}
    for cls, paths in MODEL_FILES.items():
        ...
        # The saved predictions npz was written for whichever variant the training
        # script picked (by mean PR-AUC), so it lines up with our selection here.
...
def per_fold_winners(best):
    """Pivot per-fold PR-AUC across the three model classes and report winners."""
    fold_table = {}
...
def print_interpretation(best, win_count):
    ...
    paired_diff_mean = float(np.mean(leader_fold_pr - runner_fold_pr))
    n_models = len(best)
    ...
    print(
        ...
        f"with the remaining folds split among the other models. The models genuinely "
        f"contradict each other across regimes (high-volatility 2021-22 vs calmer 2024-26 "
        f"windows have different winners), so an ensemble or a regime-aware switch is a "
        f"natural next step."
    )
...
def main():
    print("=" * 90)
    print("LOAD BEST VARIANT FROM EACH MODEL CLASS")
    print("=" * 90)
